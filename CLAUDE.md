...
**Shared workload binary for NoSQL databases** -- MongoDB and Cassandra use a single `cmd/workload/main.go` binary compiled statically (`CGO_ENABLED=0`), `docker cp`'d into the container, and executed via `docker exec`. This binary handles proper UUID generation using Go libraries and outputs JSON results for the orchestrator to parse.
...
**UUID generation:** Pre-generated by Go libraries (`github.com/google/uuid`, `github.com/oklog/ulid`), written to a file, `docker cp`'d into the container. Sysbench Lua scripts read pre-generated UUIDs from `/tmp/uuids.txt` instead of generating random bytes. This ensures proper time-ordered UUIDv7/ULID generation matching the other databases.
