...
func MySQLInsertPerformance(keyType string, numRecords, batchSize, connections int) (*benchmark.InsertPerformanceResult, error) {
	...
	if connections == 1 {
		duration, err := bench.InsertRecordsSysbench(keyType, numRecords, batchSize)
		if err != nil {
...
func MySQLReadAfterFragmentation(keyType string, numRecords, numReads int) (*benchmark.ReadAfterFragmentationResult, error) {
	bench := mysql.New()
	...
	readDuration, err := bench.ReadRecordsSysbench(keyType, numRecords, numReads)
	if err != nil {
...
func MySQLUpdatePerformance(keyType string, numRecords, numUpdates, batchSize int) (*benchmark.UpdatePerformanceResult, error) {
	...
	updateDuration, err := bench.UpdateRecordsSysbench(keyType, numRecords, numUpdates, batchSize)
	if err != nil {
...
func MySQLMixedWorkloadInsertHeavy(keyType string, totalOps, connections, batchSize int) (*benchmark.MixedWorkloadResult, error) {
	...
	result, err := bench.RunMixedWorkloadSysbench(keyType, initialDataset, totalOps, connections, 90, 10, 0)
	if err != nil {
...
func MySQLMixedWorkloadReadHeavy(keyType string, totalOps, connections int) (*benchmark.MixedWorkloadResult, error) {
	bench := mysql.New()
	...
	result, err := bench.RunMixedWorkloadSysbench(keyType, initialDataset, totalOps, connections, 10, 90, 0)
	if err != nil {
		return nil, fmt.Errorf("run mixed workload: %w", err)
	}
...
func MySQLMixedWorkloadBalanced(keyType string, totalOps, connections int) (*benchmark.MixedWorkloadResult, error) {
	bench := mysql.New()
	...
	result, err := bench.RunMixedWorkloadSysbench(keyType, initialDataset, totalOps, connections, 50, 30, 20)
	if err != nil {
		return nil, fmt.Errorf("run mixed workload: %w", err)
	}
